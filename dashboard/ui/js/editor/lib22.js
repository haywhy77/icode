(function($){		
	$.fn.editor = function(options){
		...
		return this.each(function(){
			console.log($(this))
			if(!$(this).data("editor") || $(this).data("editor")==null || $(this).data("editor")=="undefined")
				$(this).data("editor",true);
			else 
				$(this).data("editor",false);
			
			// set editor min-height to the passed height
			
			// is the status false of the editor
			if(!vars.status || !$(this).data("editor"))
			{
				...
				if($(this).closest("."+vars.css).length>0)
				{
					...
					$(this).data("editor",false).closest("."+vars.css).before(thisClone).remove();
					thisClone.replaceWith('<'+ thisElementTag + thisElementAttrs + createValue + '</'+thisElementTag+'>');
				}
				return;
			}
			
			// element will converted to the editor editor
			...
			// start editor editor to after the element
			$(this).after('<div class="'+vars.css+'"></div>');
			
			// editor
			var Richeditor = $(this).next('.'+vars.css);
			
			// insert toolbar in editor editor
			
			Richeditor.html('<div class="'+vars.css+"_toolbar"+'" role="toolbar" unselectable></div><div class="'+vars.css+'_linkform" style="display:none" role="dialog"></div><div class="'+vars.css+"_editor"+'"></div>');
			
			var toolbar			= Richeditor.find('.'+vars.css+"_toolbar"); // the toolbar variable
			var linkform		= Richeditor.find('.'+vars.css+"_linkform"); // the link-form-area in the toolbar variable
			var editor			= Richeditor.find('.'+vars.css+"_editor"); // the text-field of editor editor
			...
			var sourceField = Richeditor.find("."+vars.css+"_source"); // the source-area variable
			...
			editor.attr("contenteditable","true").css("min-height", vars.height).html(thisElementVal);
			...
			{
				Richeditor.prepend('<div class="'+vars.css+'_placeholder" unselectable><div class="'+vars.css+'_placeholder_text">'+vars.placeholder+'</div></div>');
				
				var placeHolder = Richeditor.find("."+vars.css+"_placeholder");
			...
			function replaceSelection(tTag,tAttr,tVal) {
				...
				// first, prevent to conflict of different editor editors
				if(editor.not(":focus"))
					editor.focus();
				...
				if(!toolbar.data("sourceOpened"))
				{
					...
					linkinput
						...
						.bind("keypress keyup",function(e)
						{
							if(e.keyCode==13)
							{
								linkRecord(Richeditor.find("["+setdatalink+"]"));
								return false;
							}
						});
			...
			function linkAreaSwitch(status)
			{
				...
				Richeditor.find("["+setdatalink+"][href]").removeAttr(setdatalink);
			...
			{
				Richeditor.find(elem).each(function(){
			...
			// the function of highlighting the toolbar buttons according to the cursor position in editor editor
			...
			toolbutton
					...
					else
					{
						...
						if(!toolbar.data("sourceOpened"))
						{
							...
							else
							{
								...
								// to highlight the toolbar buttons according to the cursor position in editor editor
				...
				.hover(function(e){
					if(vars.title && $(this).data("title")!="" && ( $(e.target).hasClass(vars.css+"_tool") || $(e.target).hasClass(vars.css+"_tool_icon") ))
					{
						...
						Richeditor.append('<div class="'+vars.css+'_title"><div class="'+vars.css+'_titleArrow"><div class="'+vars.css+'_titleArrowIcon"></div></div><div class="'+vars.css+'_titleText">'+$(this).data("title")+'</div></div>');
			...
			editor
				...
				.focus(function()
				{
					...
					// add onfocus class
					Richeditor.addClass(vars.css+"_focused");
				...
				.focusout(function()
				{
					...
					Richeditor.removeClass(vars.css+"_focused");
