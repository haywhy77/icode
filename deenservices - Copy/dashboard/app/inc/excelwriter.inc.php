...
EOH;
		...
		*/
		//write content header
		 function writeHeader($line_arr, $cell=2, $css = NULL){
			if($this->state!="OPENED")
			{
				$this->error="Error : Please open the file.";
				return false;
			...
			if(empty($line_arr))
			{
				$this->error="Error : Argument is not valid. Supply a valid Header.";
				return false;
			}
			$cssStr='';
			if($css!=NULL)
			...
			fwrite($this->fp,"<tr>");
			fwrite($this->fp,"<td class=xl24 width=64 $cssStr colspan=$cell>$line_arr</td>");
			fwrite($this->fp,"</tr>");
		}
